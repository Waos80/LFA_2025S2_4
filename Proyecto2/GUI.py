...
def GenerarArbol(cuenta_operaciones: int, cuenta_numeros: int, arbol: graphviz.Digraph, operacion: list) -> tuple:
    niveles = 0
    ...
    while niveles > 0:
        ...
        nivel = operacion[ultima_operacion:cierre_ultima_operacion + 1]
        tipo_operacion = ""

        if len(nivel_inferior) > 0:
            hay_inferior = True

        if hay_inferior:
            for elemento in nivel_inferior:
                arbol.edge("raiz" + str(cuenta_operaciones), elemento)
            nivel_inferior.clear()
        ...
        for token in nivel:
            ...
            if es_operacion:
                ...
                arbol.node("raiz" + str(cuenta_operaciones), tipo_operacion)
                nivel_inferior.append("raiz" + str(cuenta_operaciones))
                cuenta_operaciones += 1

            if token[0] == "numero":
                arbol.node("numero" + str(cuenta_numeros), token[1])
                arbol.edge("raiz" + str(cuenta_operaciones - 1), "numero" + str(cuenta_numeros))
                cuenta_numeros += 1
                n_nodos += 1

        

        inferior = operacion[:ultima_operacion]
        superior = operacion[cierre_ultima_operacion + 1:]
        operacion = inferior + superior
        niveles -= 1

    return (cuenta_operaciones, cuenta_numeros)

def GenerarArboles(operaciones: list[list]) -> None:
    cuenta_operaciones = 0
    cuenta_numeros = 0
    arboles: graphviz.Digraph = graphviz.Digraph(name="Jerarquias", format="png")
    for i in range(len(operaciones)):
        operacion = operaciones[i]
        with arboles.subgraph(name = "Operacion" + str(i)) as a:
            cuenta_operaciones, cuenta_numeros = GenerarArbol(cuenta_operaciones, cuenta_numeros, a, operacion)
...
def analizar():
    ...
    GenerarResultados(l)
    GenerarArboles(SepararPorOperacion(l.tokens))
